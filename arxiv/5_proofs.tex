...
\section{Additional Distant-Initialization Experiments}
\label{app:distant-initialization}

We further stress-test the particle schemes by initializing them progressively farther from the target. The target is $\pi=\mathcal{N}(1,0.7^2)$, the RBF length scale is $1$, and every run uses $N=100$ particles, seed $42$, and $100$ iterations. WFR and WIFT start from deterministic equally spaced particles, whereas the raw upstream MSIP implementation samples its initial particles on the stated interval. We use $\alpha=1$ and $\beta=0.5$ and retain the unsmoothed trajectories. Figure~\ref{fig:appendix-distant-mmd} reports the learning-rate sweep. The MSIP curve for $\tau=1$ is omitted because the upstream update encounters a singular Gram matrix in this setting.

\begin{figure}[p]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/appendix_mmd_distant_initializations.pdf}
    \caption{MMD trajectories under progressively more distant initializations on $[8,10]$, $[16,18]$, and $[32,34]$. The panels show the raw behavior of each method; values are not smoothed.}
    \label{fig:appendix-distant-mmd}
\end{figure}

Figure~\ref{fig:appendix-particle-trajectories} adapts the path-and-weight visualization of \citet[Figures~4--5]{gladin2024interaction} to the present one-dimensional experiment. Each panel displays a fixed subsample of $20$ particles in space--time coordinates. The blue background represents the target density. Intermediate point size and opacity increase with $|w^{(i)}|$, blue and red indicate positive and negative weights, orange markers show the initial particles, green markers show final particles with positive weight, and hollow red markers show final particles with nonpositive weight. To keep the paths readable, we use $\tau=0.1$ for WFR and WIFT and $\tau=0.01$ for raw MSIP.

\begin{figure}[p]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/appendix_particle_trajectories.pdf}
    \caption{Particle trajectories under progressively more distant initializations. The horizontal axis is particle location and the vertical axis is iteration.}
