...
\paragraph{Dr$^2$MMD} 
We spectrally regularize the continuous-time transport and retain the same implicit IFT weight step. For $0<\lambda\leq1$, the dynamics are
\begin{align*}\label{eq:dr2-mmd-pd}
    \dot{x}_t^{(i)}&=-\nabla \big\{ ((1-\lambda) \calT_{\mu_{N, t}} + \lambda)^{-1} h_{\bm{X}_t,\bm{w}_t} \big\} \left(x_t^{(i)}\right), \tag{$\mathrm{Dr}^{2}\,\mathrm{MMD}$-PD} \\
    \dot{w}_t^{(i)} &=  - w_t^{(i)} \left[\left((1-\lambda) \mathcal{T}_{\mu_{N, t}} + \lambda \right)^{-1} h_{X_t, w_t} \right]\left(x_t^{(i)}\right).
\end{align*}


\begin{lem}[Finite-particle implementation of Dr$^2$MMD]\label{lem:dr2-mmd-implementation}
