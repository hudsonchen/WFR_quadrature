...
&\hspace{-20pt} - \beta \mu_t \left( \calF_{\mmd}'[\mu_t]
...
\paragraph{De-regularized Fisher-Rao} 
...
\begin{align*}
    ...
    &- \beta \left( \mu_t ((1-\lambda) \calT_{\mu_t} + \lambda)^{-1} \calF_{\mmd}'[\mu_t]\right) . \tag{DrFR-GF}
\end{align*}
...
\left(x_t^{(i)} \right), \nonumber \\
w_{t+1}^{(i)}
&= w_t^{(i)} - \tau_w w_t^{(i)} \cdot h_{\bm{X}_{t+1}, \bm{w}_t}
...
\paragraph{De-regularized MMD} 
Motivated by the spectral regularization of the weight update scheme in WIFT, we next introduce a spectral regularization on the particle update scheme as well. 
\begin{align}\label{eq:drfr-pd}
    x_{t+1}^{(i)} &= x_t^{(i)}-\tau_X \nabla\left[\left((1-\lambda) \mathcal{T}_{\mu_{N, t}}+\lambda \right)^{-1} h_{\bm{X}_t, \bm{w}_t} \right]\left(x_t^{(i)}\right), \tag{DrFR-PD} \\
    \bm{w}_{t+1}
    &= \frac{1}{1+\tau_w} \bm{w}_t
    + \frac{\tau_w}{1+\tau_w}
    \bm{w}_{\mathrm{KQ}}(\bm{X}_{t+1}) \nonumber . 
...
Then,
\begin{enumerate}
    \item $\calE(\cdot,\bm{w})$ is $\max_{i\in[N]} w^{(i)}(2 C_1 + C_2)$-smooth under the standard Euclidean geometry; and is $(2 C_1 + C_2)$-smooth under the $\operatorname{Diag}(\bm{w})$-weighted Euclidean geometry. 
...
In the next proposition, we analyze the particle descent schemes introduced above, identify the geometry underlying each scheme, and compare their corresponding admissible step sizes for monotonic desecent. 
...
\begin{enumerate}
    \item For E-PD, the sufficient location condition is $\tau_X\leq 2/[\max_{i\in[N]} w_t^{(i)}(2C_1+C_2)]$. The sufficient weight condition is $\tau_w\leq 2/\lambda_{\max}(K_{t+1})$.
...
For the step size condition that $\tau_X\leq 2/[\max_{i\in[N]} w_t^{(i)}(2C_1+C_2)]$, the maximal admissible step size is dictated by the largest particle weight. Consequently, particles with smaller weights suffers from a factor of $[\min_{i\in[N]} w_t^{(i)}] / [\max_{i\in[N]} w_t^{(i)}]$. 
For the step size condition that $\tau_w\leq 2/\lambda_{\max}(K_{t+1})$, 
